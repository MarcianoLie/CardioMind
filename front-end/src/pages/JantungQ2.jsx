...
const JantungQ2 = () => {
    const [weight, setWeight] = useState(sessionStorage.getItem("q2_weight") || "");
    const [height, setHeight] = useState(sessionStorage.getItem("q2_height") || "");
    const navigate = useNavigate();

    useEffect(() => {
        // Save input values when they change
        const saveWeight = () => {
            sessionStorage.setItem("q2_weight", weight);
        };
        const saveHeight = () => {
            sessionStorage.setItem("q2_height", height);
        };
